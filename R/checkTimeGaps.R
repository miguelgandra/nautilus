...
#'
#' @param data A data frame containing a column with datetime information. The column should be of class `POSIXt` 
...
#' 
#' @return A data frame (`data`) with rows before or after the detected temporal discontinuities removed, based on the thresholds specified.
#' If the gap is detected within the specified range (before or after the deployment), the corresponding data will be retained.
#' 
...
checkTimeGaps <- function(data,
  ...
  # Convert the mode difference to a more human-readable format (optional)
  interval_readable <- paste0(interval, " secs")
