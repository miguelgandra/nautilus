...
#'
#' @details
#' The function supports multiple types of position data:
#' \itemize{
...
#'   \item \code{"Metadata [deploy]"}: deployment coordinates from tag metadata
#'   \item \code{"Metadata [popup]"}: pop-up location from tag metadata
#'   }
#'
...
#'   \item User and Metadata \[deploy\] positions always preserved
...
filterLocations <- function(data,
  ...
  for (i in 1:length(data)) {
    ...
    # order by datetime
    data.table::setorderv(dt, datetime.col)
    ...
    # get first metadata deploy position with valid coordinates
    metadata_deploy_pos_in_data <- dt[get(position.type.col) == "Metadata [deploy]" & !is.na(get(lon.col)) & !is.na(get(lat.col)),
                                      .(lon = get(lon.col)[1], lat = get(lat.col)[1])]

    # check metadata for deployment coordinates
    metadata_row <- id.metadata[id.metadata[[id.col]] == id, ]
    metadata_pos_from_id_metadata <- NULL
    if (nrow(metadata_row) > 0) {
      ...
      if (!is.na(deploy_lon) && !is.na(deploy_lat)) {
        metadata_pos_from_id_metadata <- data.table::data.table(lon = deploy_lon, lat = deploy_lat)
      }
    }
    ...
    if (nrow(user_pos) > 0) {
      deploy_pos <- user_pos
      # Compare with metadata from id.metadata (if available)
      if (!is.null(metadata_pos_from_id_metadata)) {
        dist <- geosphere::distHaversine(as.numeric(user_pos), as.numeric(metadata_pos_from_id_metadata)) / 1000
        if (dist > 1) warning_msg <- c(warning_msg, sprintf("First 'User' position differs from 'id.metadata' deployment by %.2f km", dist))
      }
      # Compare with 'Metadata [deploy]' from data (if available)
      if (nrow(metadata_deploy_pos_in_data) > 0) {
        dist <- geosphere::distHaversine(as.numeric(user_pos), as.numeric(metadata_deploy_pos_in_data)) / 1000
        if (dist > 1) warning_msg <- c(warning_msg, sprintf("First 'User' position differs from 'Metadata [deploy]' by %.2f km", dist))
      }
    } else if (nrow(metadata_deploy_pos_in_data) > 0) {
      deploy_pos <- metadata_deploy_pos_in_data
      # Compare with metadata from id.metadata (if available)
      if (!is.null(metadata_pos_from_id_metadata)) {
        dist <- geosphere::distHaversine(as.numeric(metadata_deploy_pos_in_data), as.numeric(metadata_pos_from_id_metadata)) / 1000
        if (dist > 1) warning_msg <- c(warning_msg, sprintf("'Metadata [deploy]' position differs from 'id.metadata' deployment by %.2f km", dist))
      }
    } else if (!is.null(metadata_pos_from_id_metadata)) {
      deploy_pos <- metadata_pos_from_id_metadata
    } else {
      warning_msg <- c(warning_msg, sprintf("No valid 'User', 'Metadata [deploy]' positions or 'id.metadata' coordinates available for ID: %s.", id))
    }
    ...
    total_metadata_deploy <- sum(dt[[position.type.col]] == "Metadata [deploy]", na.rm = TRUE)
    total_metadata_popup <- sum(dt[[position.type.col]] == "Metadata [popup]", na.rm = TRUE) # Changed from pop to popup
    ...
    if (!is.null(min.satellites)) {
      invalid_sat <- which((dt[[position.type.col]] %in% c("FastGPS", "Metadata [popup]")) & !is.na(dt[[quality.col]]) & dt[[quality.col]] < min.satellites) # Changed from pop to popup
    ...
    if(is.null(deploy_pos) || nrow(deploy_pos) == 0) {
      ...
      # identify spurious distances (excluding "User" and "Metadata [deploy]" positions)
      invalid_dist <- which((dt[[position.type.col]] %in% c("FastGPS", "Metadata [popup]")) & distances > max.distance.km & !is.na(dt[[position.type.col]])) # Changed from pop to popup
      ...
      if (length(position_idx) > 1) {
        repeat {
          ...
          distances <- geosphere::distGeo(
            cbind(lons[-length(lons)], lats[-length(lons)]),
            cbind(lons[-1], lats[-1])) / 1000

          # calculate speeds in km/h
          speeds <- distances / time_diffs

          # identify pairs where speed exceeds threshold (only consider pairs involving filterable types)
          filterable_types <- c("FastGPS", "Metadata [popup]") # Changed from pop to popup
          speed_violations <- which(speeds > max.speed.kmh &
                                            (dt[[position.type.col]][position_idx[-length(position_idx)]] %in% filterable_types |
                                               dt[[position.type.col]][position_idx[-1]] %in% filterable_types))
          ...
          for (j in speed_violations) {
            ...
            # only consider filterable positions for removal
            filterable_outliers <- outlier_candidates[dt[[position.type.col]][outlier_candidates] %in% filterable_types]

            # if we found filterable outliers, select the one with worst quality (or first if equal)
            if (length(filterable_outliers) > 0) {
              if (!is.null(quality.col) && all(!is.na(dt[[quality.col]][filterable_outliers]))) {
                # remove the one with lowest quality
                to_remove <- c(to_remove, filterable_outliers[which.min(dt[[quality.col]][filterable_outliers])])
              } else {
                # default to removing the first one if no quality info
                to_remove <- c(to_remove, filterable_outliers[1])
              }
            }
          }
    ...
    # Changed Metadata [deploy] to Deploy and Metadata [popup] to Popup in output string
    cat(sprintf("Total positions: %d [User: %d, Fastloc: %d, Deploy: %d, Popup: %d]\n",
                            total_pos, total_user, total_fastloc, total_metadata_deploy, total_metadata_popup))
    if(total_flags == 0 & length(warning_msg)==0){
      cat("\u2713 All positions meet quality standards\n")
    }else{
      if(length(warning_msg)>0) {
        cat("Warning(s):\n")
        for(msg in warning_msg) cat(paste0("  - ", msg, "\n"))
      }
      if (total_flags > 0) {
    ...
    if (plot && (total_flags>0 | length(warning_msg)>0)) {
      ...
      # get row indices with valid locations from original data
      ...
      # determine all deployment markers for plotting range
      all_deploy_lons <- c()
      all_deploy_lats <- c()
      if (nrow(user_pos) > 0) {
        all_deploy_lons <- c(all_deploy_lons, user_pos$lon)
        all_deploy_lats <- c(all_deploy_lats, user_pos$lat)
      }
      if (nrow(metadata_deploy_pos_in_data) > 0) {
        all_deploy_lons <- c(all_deploy_lons, metadata_deploy_pos_in_data$lon)
        all_deploy_lats <- c(all_deploy_lats, metadata_deploy_pos_in_data$lat)
      }
      if (!is.null(metadata_pos_from_id_metadata)) {
        all_deploy_lons <- c(all_deploy_lons, metadata_pos_from_id_metadata$lon)
        all_deploy_lats <- c(all_deploy_lats, metadata_pos_from_id_metadata$lat)
      }

      # calculate ranges including relevant deployment locations
      range_lons <- c(orig_lon, new_lon, all_deploy_lons)
      range_lats <- c(orig_lat, new_lat, all_deploy_lats)

      lon_range <- grDevices::extendrange(stats::na.omit(range_lons), f = 0.2)
      lat_range <- grDevices::extendrange(stats::na.omit(range_lats), f = 0.2)
      ...
      graphics::plot(NA, xlim = lon_range, ylim = lat_range, asp = 1, las = 1, axes = FALSE,
                     xlab = "", ylab = "", xaxs = "i", yaxs = "i")

      # add title and axes labels
      graphics::title(main = paste(id, "- Positions"), line= 2, cex.main = 0.9)
      deploy_duration <- as.numeric(difftime(max(data[[i]][[datetime.col]]), min(data[[i]][[datetime.col]]), "hours"))
      graphics::title(main = sprintf("[Deployment: %.2f hours] ", deploy_duration), line = 0.9, font.main=1, cex.main = 0.8)
      graphics::title(xlab = "Longitude", line = 2.2, cex.lab = 0.9)
      graphics::title(ylab = "Latitude", line = 3.2, cex.lab = 0.9)

      # add background color
      graphics::rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4], col="#D6E6F2")

      # add custom axes with pretty labels
      graphics::axis(1, at = pretty(lon_range, n = 5), labels = sprintf("%.2f", pretty(lon_range, n = 5)), cex.axis = 0.9)
      graphics::axis(2, at = pretty(lat_range, n = 5), labels = sprintf("%.2f", pretty(lat_range, n = 5)), las=1, cex.axis = 0.9)
      ...
      # set color palette
      color_pal <- grDevices::adjustcolor(c("darkorchid", "maroon2", "blue", "forestgreen", "red"), alpha.f=0.8) # Added color for Metadata [popup]

      # plot deployment locations (order matters for visibility)
      # User positions
      if (nrow(user_pos) > 0) {
        graphics::points(user_pos$lon, user_pos$lat, pch = 24, bg = color_pal[1], lwd = 0.4, cex = 1.5)
      }
      # Metadata [deploy] positions from data
      if (nrow(metadata_deploy_pos_in_data) > 0) {
        graphics::points(metadata_deploy_pos_in_data$lon, metadata_deploy_pos_in_data$lat, pch = 24, bg = color_pal[2], lwd = 0.4, cex = 1.5)
      }
      # Metadata positions from id.metadata
      if (!is.null(metadata_pos_from_id_metadata)) {
        graphics::points(metadata_pos_from_id_metadata$lon, metadata_pos_from_id_metadata$lat, pch = 24, bg = color_pal[3], lwd = 0.4, cex = 1.5)
      }

      # plot valid FastGPS and Metadata [popup] positions
      valid_filtered_idx <- which(!is.na(orig_lon) & !is.na(new_lon) & (pos_type %in% c("FastGPS", "Metadata [popup]"))) # Changed from pop to popup
      graphics::points(new_lon[valid_filtered_idx], new_lat[valid_filtered_idx], bg = color_pal[4], pch = 21, lwd = 0.4, cex = 1.5)

      # plot removed positions
      removed_idx <- which(!is.na(orig_lon) & is.na(new_lon))
      graphics::points(orig_lon[removed_idx], orig_lat[removed_idx], bg = color_pal[5], pch = 21, lwd = 0.4, cex = 1.5)

      # add location numbers
      all_indices <- 1:length(orig_lon)
      graphics::text(orig_lon, orig_lat, labels = all_indices, col = "white", cex = 0.5, font = 2)
      ...
      if (nrow(user_pos) > 0) {
        legend_labels <- c(legend_labels, sprintf("User (n=%d)", total_user))
        legend_colors <- c(legend_colors, color_pal[1])
        legend_pch <- c(legend_pch, 24)
      }
      if (nrow(metadata_deploy_pos_in_data) > 0) {
        legend_labels <- c(legend_labels, sprintf("Metadata [deploy] (n=%d)", total_metadata_deploy))
        legend_colors <- c(legend_colors, color_pal[2])
        legend_pch <- c(legend_pch, 24)
      }
      if (!is.null(metadata_pos_from_id_metadata)) {
        legend_labels <- c(legend_labels, "id.metadata Deployment")
        legend_colors <- c(legend_colors, color_pal[3])
        legend_pch <- c(legend_pch, 24)
      }

      legend_labels <- c(legend_labels,
                         sprintf("Filtered Positions (n=%d)", sum(!is.na(new_lon) & (pos_type %in% c("FastGPS", "Metadata [popup]")))), # Changed from pop to popup
                         sprintf("Removed (n=%d)", length(removed_idx)))
      legend_colors <- c(legend_colors, color_pal[4], color_pal[5])
      legend_pch <- c(legend_pch, 21, 21)

      # draw legend
      graphics::legend("right", inset = c(-0.28, 0),
  ...
  cat(crayon::bold("Total execution time:"), sprintf("%.02f", as.numeric(time.taken)), units(time.taken), "\n\n")
