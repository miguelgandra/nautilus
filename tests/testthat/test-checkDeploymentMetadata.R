# Tests for checkDeploymentMetadata(): role-gated metadata QC + normalisation.
...
run_qc <- function(raw, columns, ...) {
  suppressWarnings(checkDeploymentMetadata(raw, columns = columns, verbose = 0, ...))
}
...
test_that("a clean table passes and returns a nautilus_deployments object", {
  ...
  expect_equal(nrow(issues(out, severity = "error")), 0L)
...
test_that("required-only mapping disables package/logger/recovery checks (role-gating)", {
  ...
  expect_setequal(unique(issues(out)$check), character(0))   # nothing flagged
...
test_that("recovery before deployment is an error (needs recovery role)", {
  ...
  expect_true("recovery_before_deploy" %in% issues(out)$check)
})
...
test_that("out-of-range coordinates are flagged", {
  ...
  out <- run_qc(raw, .qc_cols())
  expect_true("implausible_location" %in% issues(out, severity = "error")$check)
})
...
test_that("overlapping deployments on one package are flagged (needs package + recovery)", {
  ...
  expect_true("package_overlap" %in% issues(out, severity = "error")$check)
})
...
test_that("identifier whitespace is normalised and reported as info", {
  ...
  out <- run_qc(raw, .qc_cols())
  expect_true("identifier_normalized" %in% issues(out)$check)
...
test_that("issues errors on a non-deployments object", {
  expect_error(issues(data.frame(a = 1)), "nautilus_deployments")
})

test_that("verbose = 0 emits an aggregated warning only when errors exist", {
  raw_bad <- .qc_raw(); raw_bad$id[2] <- raw_bad$id[1]
  expect_warning(checkDeploymentMetadata(raw_bad, columns = .qc_cols(), verbose = 0), "error")
  raw_ok <- .qc_raw()
  expect_silent(checkDeploymentMetadata(raw_ok, columns = .qc_cols(), verbose = 0))
})
...
test_that("an invalid exclude_sensors token is an error issue, not an abort", {
  ...
  out <- run_qc(raw, .qc_cols(exclude_sensors = "bad"))
  expect_true("invalid_exclude_sensors" %in% issues(out, severity = "error")$check)
})
